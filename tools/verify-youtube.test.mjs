...
test("production Party Tyme removals are catalog-only unassignments with preserved IDs and no replacement IDs", async () => {
  ...
  assert.equal(catalog.filter((song) => song.youtubeVideoId !== null).length, 151);
  assert.equal(new Set(catalog.filter((song) => song.youtubeVideoId).map((song) => song.youtubeVideoId)).size, 151);
});
